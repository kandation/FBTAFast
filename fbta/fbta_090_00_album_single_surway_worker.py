...
class FBTA090AlbumSingSurveyWorker(FBTAMainWorker):
    ...
    # def find12img(self, url_new, source):
    #     data_ret = {
    #         'url': str(url_new),
    #         'source': '',
    #         'is-more': '',
            ...
            'photo-cluster':
                {
                    'url': str(url),
                    'source': source,
                    'download-time': time.time() - timer_dl_start,
                }

        }

        # data_insert = {'photo-cluster': [self.find12img(url, source)], }

        print(f":AlbumSGSWorker: Get Album Photos Single {aid} @{data_insert['photo-cluster']['download-time']}")
